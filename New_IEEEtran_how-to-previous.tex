...
To incorporate the uncertain variability characteristic from converter-interfaced generators into frequency dynamic. Ref. 8 treated the uncertain variability factors as an ito process that is expressed by stochastic differential equations. Since the uncertain factors in the ito process are broadly like a Gaussian distribution following good conjugation properties, the frequency dynamic derived from the ito process keeps an analytical solution. These uncertain variability factors usually depend on frequency derivations; the suggested distribution of uncertain variability characteristics is conditional and unknown. From this perspective, existing methods considering uncertain variability factors based on the ito process cannot reflect the dependable uncertain variability factors on frequency dynamic.

It is essential to review critical frequency characteristics for scheduled problems like unit commitment, optimal power flow, and economic dispatch to meet frequency security requirements. Ref.7 constructed RoCoF, nadir, and Qss constraints derived from analytical SFR models and developed a frequency-constrained unit commitment (FCUC). However, the challenge with FCUC and other similar frequency-constrained dispatch models is linearising nonlinear frequency constraints. To address this, Ref.8 constructed a max-affine-derived linear frequency nadir constraint and incorporated it into FCUC. Ref.9 utilized a specific neural network to establish linear regression relationships between maximum frequency derivation and various frequency-regulating parameters. Ref.10 applied the most miniature square model to calculate the regression parameters of the derived linearized nadir constraint. Alternatively, Ref. 10 sorted out a sequence of discretized frequency derivation constraints over a predetermined simulated horizon. Since these frequency derivation constraints for each time slice must consist of RoCoF, nadir, and Qss types, this method usually has a high level of accuracy yet with more complexity. While these multiple generators participating in SFR usually keep an apparent P-F characteristic. This means that the uncertain variability characteristic of intermittent resources is implicitly ignored throughout the dynamic response process, although their P-F characteristic has been reconsidered with free-noise assumptions.

Motivated by such concerns, this work constructs a new SFR model considering uncertain variability from converter-interfaced generators. Since the uncertain variability characteristic retreats as a stochastic waveform sequence within a Bayesian-filter framework that can be measured by phase measurement unit (PMU), the trajectory of frequency derivation through the proposed model is more accurate with the help of PMU data. In addition, the probabilistic density distribution concerning frequency derivation considering uncertain variability can also be derived. Subsequently, the chronological derivation and related probabilistic distribution of frequency dynamic are abstracted, reformulated as chance-constrained frequency constraints, and integrated into UC studies. Accordingly, a refined chance-constrained FCUC is developed.

To summarize, two contributions to this work are described below,
1) a new SFR model that considers uncertain variability from converter-interfaced generators is proposed. The proposed model can estimate frequency derivation and associated probabilistic distribution against mismatch power disturbance.
...
The remainder of this paper is organized: Section \Rmnum{2} introduces a Bayesian-filter-derived SFR model. Section \Rmnum{3} constructs chance-constraint frequency dynamic constraints. \Rmnum{Four} extends the formulation of chance-constraint FCUC. Section \Rmnum{5} verifies the effectiveness of the proposed model in a modified IEEE 14 bus test system and IEEE 118 bus test system, and Section \Rmnum{6} summarizes the primary findings and conclusions presented in this research.

\vspace{-0.125in}
\section{Modeling BF-SFR through Bayesian-Filter}

% Considering a low-inertia grid comprised of conventional generators, wind farms and BESS, it is seen that the effect of homogeneous generators with comparable control schemes can be seen as an aggregated CoI block. Define aggregated parameters $\displaystyle\left\{H_{G}, D_{G}, K_{G}, F_{G}, R_{G}, T_{G}\right\},\left\{K_{W}, R_{W}, M_{W}, D_{W}, T_{W}\right\}$ and $\left\{T_{E}, R_{E}\right\}$ represent the adjusted output produced by CoI conventional generator block, CoI wind farm block and CoI BESS block. Further details on the derivation from frequency parameters of individual units to aggregated unit block can be found in \cite{restrepo2005unit, teng2015stochastic, chu2020towards, ding2021two,zhang2020modeling,zhang2021encoding}.
...
Different generators usually have heterogeneous control schemes for primary frequency dynamics against mismatch power disturbance cases. For simplification, homogeneous generators with similar controls usually equivalent units with aggregated parameters. Considering such a power system consisting of conventional generators, converters-interfaced generators and battery energy systems (BESS), the union frequency derivation for the system can be written as,

% \vspace{-0.125in}
\begin{small}
	\begin{align}\label{eqn:1}
		 & \left(M_{G} s+D_{G}\right) \varDelta f(s)=\frac{1}{s} \varDelta p_{ {step }}- \left(\frac{K_G\left(1+F_{G} T_{G} s\right)}{R_G\left(1+T_{G} s\right)}\right. \\
		 & + \frac{K_W}{R_{W}(1+T_{W}s)} + \frac{K_W(H_Ws +D_W)}{1+T_Ws} + \left. \frac{1}{R_E(1+T_{E} s)}\right) \varDelta f(s) \notag
	\end{align}
\end{small}
...
\begin{newsubequations}\label{eqn:2}
	\begin{align}
		M_{\varSigma } \varDelta f^{\prime}(t) & +D_{\varSigma} \varDelta f(t)=\varDelta p_{mis}(t)                                                                                        \\
		\varDelta p_{mis}(t)                         & =\varDelta p_{step}-\varDelta p_{G}(t)-\varDelta p_{W}(t)-\varDelta p_{E}(t)                                                        \\
		M_{\varSigma}                          & =M_{G}+M_{W}                                                                                                                        \\
		D_{\varSigma}                          & =D_{G}+D_{W}+{ R_{W}^{-1} } + { R_{E}^{-1}}                                                                                         \\
		\varDelta p_{G}(t)                     & ={K_{G} F_{G}}{ R_{G}^{-1}} \varDelta f(t)- \notag                                                                                  \\
		                                       & \quad\,\,\, {K_G(1-F_G)}{ R_{G}^{-1} }e^{-\frac{1}{T_G}t} \int_{0}^{t}e^{\frac{1}{T_G}\kappa} \varDelta f^{\prime}(\kappa) d \kappa \\
		\varDelta p_{W}(t)                     & =\left({R_{W}^{-1}}\right) \varDelta f(t)                                                                                           \\
		\varDelta p_{E}(t)                     & =\left( { R_{E}^{-1}}\right) \varDelta f(t)
	\end{align}
\end{newsubequations}
%\begin{figure}[!t]
%    \centering
...
By replacing $\varDelta f^{\prime}(t)|_{t = t}$ as its equivalent discretized form $\sfrac{[\varDelta f(t_0+ n\delta t)-\varDelta f(t_0 + (n-1)\delta t)]}{ \delta t}$, the time-discretized form of $\varDelta f(t)$ can be estimated,

% \vspace{-0.125in}
\begin{newsubequations}\label{eqn:3}
	\begin{align}
		 & \varDelta p_{G}(t)={K_{G} }{ R_{G}^{-1}} \varDelta f(t) - \varLambda \int_{0}^{t} e^{\frac{1}{T_G}(t)} \label{time-discretized SFR: additional power from convertional units} \\
		 & \left(\varDelta f(t+\delta t) - \varDelta f(t) \right) d t: \varLambda = {K_G(1-F_G)}{ R_{G}^{-1} }e^{-\frac{1}{T_G} (t)} \notag                                              \\
	\end{align}
\end{newsubequations}
...
Since uncertain variability factors are generally conditionally depend on online frequency derivation and itself actual output restriction, a common-based expression of uncertain variability factors can be written as \(\varDelta \omega(t) = \lceil H(\varDelta f(t)), \overline{p}_{add,W}\rceil + \sigma_{add,W}\). By integrating \(\varDelta \omega\) into (2), the form of frequency dynamic considering uncertain variability factors can be obtained as below,

% \vspace{-0.125in}
\begin{newsubequations}\label{eqn:3}
	\begin{align}
		\varDelta f(t + \delta t) & =\varDelta f(t)+M_{\varSigma}^{-1}\bigl(\varDelta p_{mis}(t)-D_{\varSigma} \varDelta f(t)\bigr) \delta t + \delta \omega\delta t \label{time-discretized SFR: swing equation} \\
		\varDelta p_{mis}(t)            & =\varDelta p_{{step}}-\varDelta p_{G}(t)-\varDelta p_{W}(t)-\varDelta p_{E}(t) \label{time-discretized SFR: mismatch power}                                             \\
	\end{align}
\end{newsubequations}

In (4), the stochastic differential equation does not have an analytical solution due to the conditional dependency of uncertain variability factors. To address this issue, a Bayesian filter framework is introduced to estimate the posterior distribution of frequency derivation. Let \(\varDelta f(t) \Rightarrow \varDelta f(t + \delta t)\) be a Markov decision process, and \(\varDelta p(t) \Rightarrow \varDelta p(t+\delta t)\) de a measured state observable process, the calculation of \(\varDelta f(t + \delta t)\) in (4) can be solved alternately, as illustrated in figure.(2).
...
\begin{newsubequations}
	\begin{align}
		 & \mathcal{P}(\varDelta f(t+\delta t) | \varDelta p_{mis}(t_0: t+\delta t; \mu_1,\sigma_1))=                                                                                                                                                                            \\
		 & \frac{\mathcal{P}(\varDelta f(t+\delta t) | \varDelta p_{mis}(t_0: t; \mu_1,\sigma_1)) \mathcal{P}(\varDelta p_{mis}(t+\delta  t) | \varDelta f(t+\delta t; \mu_1,\sigma_1))}{\mathcal{P}(\varDelta p_{mis}(t+\delta t) | \varDelta p_{mis}(t_0: t; \mu_2,\sigma_2))}
	\end{align}
\end{newsubequations}
...
\noinedent where \cref{time-discretized SFR: swing equation} declares the change of frequency derivation between adjacent discretized steps; \cref{time-discretized SFR: mismatch power} describes chronological mismatch power; \cref{time-discretized SFR: additional power from conventional units,time-discretized SFR: additional power from converters,additional power from bess}  enforces rescheduled outputs produced by aggregated conventional units, aggregated wind units and aggregated battery energy units.

In the above equation, \(\varDelta f(t + \delta t)\) can be indirectly solved by two steps: 1) a Bayesian-derived inference process, and 2) a updatable prediction process. At the inference step, the prior value of \(\varDelta f(t + \delta t)\) is estimated on the basis of \(\langle \varDelta f( \lambda), \varDelta p_{mis}( \lambda )\rangle|_{t\in\{t_0, t\}}\); while at the latter step,
...
In \cref{eqn:4}, \(\mathcal{P}(\varDelta f(t_0\!+\!(n\!+\!1)\delta t \) \(\!\mid\! \varDelta p\left(t_{0}: t_0\!+\!(n\!+\!1)\delta t\right), u_{1}$, $\sigma_{1})\)  can be indirectly calculated by estimating its prior form $\mathcal{P}\left(\varDelta f(t_0 +(n+1)\delta t) \!\mid\! \varDelta p\left(t_{0}\!: \! t\right), u_{1}, \sigma_{1}\right)$ and a related normalized constant $\mathcal{P}\left(\varDelta p(t_0 +(n+1)\delta t) \mid \varDelta f\left(t_0 +\right.\right.$ $\left.\left.  (n+1)\delta t\right),  u_{2}, \sigma_{2}\right)$, with their forms can be found in \cref{eqn:5}.

% \vspace{-0.125in}
\begin{newsubequations}\label{eqn:5}
	\begin{align}
		 & \mathcal{P}\left(\varDelta f(t_0\!+\!(n\!+\!1)\delta t) \!\mid\! \varDelta p\left(t_{0}\!:\! t_0\!+\!n\delta t\right), \!u_{1},\! \sigma_{1}\right)=                                                                     \\
		 & \int \bigl\{\mathcal{P}(\varDelta f(t_0\!+\!(n\!+\!1)\delta t) \!\mid\! \varDelta f(t_0\!+\!n\delta t), \left.\!\mu_{1}, \!\sigma_{1}\right)\!\mathcal{P}\left(\varDelta f(t_0 +\!\right. \notag                         \\
		 & \qquad\,\, \left. \!n\delta t) \!\mid\!  \varDelta p\left(t_{0}: t_0\!+\!n\delta t\right), u_{1}, \sigma_{1}\right)\bigr\} \cdot d \varDelta f(t_0 \!+\! n\delta t)\notag                                                \\
		 & \mathcal{P}\left(\varDelta p(t_0\!+\!(n\!+\!1)\delta t) \!\mid\!  \varDelta p\left(t_{0}\!:\! \right.\right. \left. \left.  t_0\!+n\delta t\right), u_{2}, \sigma_{2}\right) =                                           \\
		 & \int \bigl\{\mathcal{P}(\varDelta p(t_0+(n+1)\delta t) \!\mid\!  \varDelta f(t_0+(n+1)\delta t)\left.u_{2}, \sigma_{2}\right) \notag                                                                                     \\
		 & \mathcal{P}\!\left(\varDelta \!f(t_0\!+\!(n\!+\!1)\!\delta t) \!\mid\!  \varDelta p\!\left(t_{0}\!: \! t_0\!+\!n \delta t\right), \!u_{2}, \!\sigma_{2}\right) \bigr\} d \varDelta f\!(t_0\!+\!(n\!+\!1)\delta t) \notag
	\end{align}
\end{newsubequations}
...
\begin{newsubequations}\label{eqn:7}
	\begin{align}
		 ...
		 & \quad\quad\qquad\qquad N^{-1}\sum\nolimits_{n=1}^{N} \mathcal{P}\left(\varDelta f(n+1) \mid \varDelta f(n), u_{1}, \sigma_{1}\right)\lambda_{n}^{\prime}\notag                      \\
		 & \quad\quad\qquad\qquad\lambda_{n}^{\prime} \sim \mathcal{P}\left(\varDelta f(n) \!\mid\!  \varDelta p\left(0: n\right), u_{1}, \sigma_{1}\right)                                    \\
		 & \mathcal{P}\left(\varDelta p(n\!+\!1) \!\mid\! \varDelta p\left({0}\!:\! n\right), u_{2}, \sigma_{2}\right) \!=\label{eqn-7: sequential relation of adjuctable outputs}             \\
		 & \quad\quad\qquad\qquad N^{-1}\sum\nolimits_{n=1}^{N} \mathcal{P}\left(\varDelta p(n+1) \mid \varDelta f(n+1), u_{2}, \sigma_{2}\right)\lambda_{n}^{\prime\prime} \notag             \\
		 & \quad\quad\qquad \qquad\lambda_{n}^{\prime\prime} \sim \mathcal{P}\left(\varDelta f(n+1) \!\mid\!  \varDelta p\left(0: n\right), u_{2}, \sigma_{2}\right)
	\end{align}
\end{newsubequations}

\noindent where $\lambda^{\prime}, \lambda_{n}^{\prime \prime}$, respectively, represent random filters following associated integrand functions in (7b) and (7d). In combination with \eqref{eqn-7: sequential relation of adjacent frequency derivations} and \eqref{eqn-7: sequential relation of adjuctable outputs}, the posterior probabilistic assessment of $\varDelta f(n+1)$ concerning $\mathcal{P}(\varDelta f(n+1))$ can be carried out with the below steps:
\begin{itemize}
...
Substituting $\iota(n+1) = \lambda (n+1)\gamma(n+1), \iota(n) = \lambda (n)\gamma(n)$ into \eqref{eqn:4}, $\iota(n+1)$ can be recursively calculated through \eqref{eqn:8}. In \eqref{eqn:8}, another critical issue to be reviewed is the selection of the proposed distribution. Here, the recursive transfer distribution $\iota(\varDelta f(n+1)|\varDelta f(n),\mu_1,\sigma_1)$ can be availability used to simplify the calculation process. Accordingly, the updating weight \eqref{eqn:8} can be reduced as,
...
In \cref{eqn:18}, the inner $``\max"$ operator automatically selects the most suitable hyperplane closet to each evaluation point, and the outer $``\min"$ operator guarantees the global accuracy between pre-selected hyperplane and all evaluation points. In the term of problem solving, \eqref{eqn:18} can be solved by directly calling \code{Ipopt} solver or reformulating it as its recursive form and subsequently invoking existing MIP solver, e.g., \code{Gurobi}, \code{IBM CPLEX}, etc. More detailed on the calculation of \eqref{eqn:18} can be found in \cite{ding2021two,zhang2020modeling}
