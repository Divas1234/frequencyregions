...
\begin{newsubequations}
	\begin{small}
		\begin{align}
			\left(\varDelta f(t)\right)^{(n)} - M \tau_{n}\!              & \leq \!\varDelta f_{nadir}, \label{eqn:14a}                             \\
...
\begin{newsubequations}
	\begin{small}
		\begin{align}
			&\left(\varDelta f(t)\right)^{(n)} \!-\! M \tau_{n}\!  \leq \!\!\!\!\!\!\! \max\limits_{m\in\{1,2,...,m,...,M\}}\left(\alpha_{m}^{(1)} H_{\varSigma} +  \alpha_{m}^{(2)} D_{\varSigma} + \right.\label{eqn:17a}\\
			&\left. \alpha_{m}^{(3)} \left(\frac{F_{G}}{R_{G}}\right)+\alpha_{m}^{(4)}\right), \forall n \in\{1,2,...n,...,N\},\forall t\in(t_{IR}, t_{PFR}]  \notag             \\
...
In the above formulation, Eqs. \eqref{20-a} -\eqref{20-v} define a baseline UC model, \cite{zhang2020modeling}, Eqs. \eqref{21-a}-\eqref{21-l} denote frequency dynamic constraints in line with the actual frequency requirements. It is worth noting that the newly redesigned nadir constraint \eqref{21-e}-\eqref{21-f} is based on BF-SFR, which is significantly different from the previous SFR approach or UC studies.
