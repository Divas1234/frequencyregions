...
The stochastic differential equation (SDE) defined in \cref{eqn:4} does not have an analytical solution due to the independence between uncertain variability factors and \(\varDelta f(t)\). To address this issue, a Bayesian filter framework is introduced to estimate the posterior distribution of \(\varDelta f(t)\). Let \(\langle\mathcal{P} (\varDelta f(t)), \mathcal{P}(\varDelta p(t))\rangle\) respectively enforces the probabilistic density distribution of \(\varDelta f(t)\) and \(\varDelta p(t)\), then, the posterior distribution of \(\varDelta f(t)\) can be estimated by,
...
\begin{newsubequations}\label{eqn:5}
	\begin{align}
		 & \mathcal{P}(\varDelta f(t) | \varDelta p(t_0: t; \varDelta \omega))=\frac{1}{ {\mathcal{P}(\varDelta p(t) | \varDelta p(t_0: t-\delta t);\varDelta \omega)}}\cdot \notag                                                                                         \\
		 & \quad \quad  \underbrace{\mathcal{P}(\varDelta p(t) | \varDelta f(t); \varDelta\omega)}_{\textrm{\textit{likelihood function}}}\,\, \underbrace{\mathcal{P}(\varDelta f(t) | \varDelta p(t_0: t-\delta t); \varDelta \omega)}_{\textrm{\textit{prior function}}}
	\end{align}
\end{newsubequations}
...
\noindent where the \(\textit{likelihood function}\) in \cref{eqn:5} declares the coupling characteristic between \(\varDelta p(t)\) and \(\varDelta f(t)\); the \(\textit{prior function}\) in \cref{eqn:5} describes the recursive process from \(\varDelta f(t_0: t-\delta t)\) to \(\varDelta f(t)\). Using the prior and predicated steps of Bayesian filter written probabilistically, uncertain variability factors \(\varDelta \omega\) is taken into consideration over the entire frequency dynamic process.
More detailed, both \(\textit{likelihood function}\) and \(\textit{prior function}\) can be recursively calculated as follows,

% \vspace{-0.125in}
\begin{newsubequations}\label{eqn:5}
	\begin{align}
		 & \mathcal{P}\left(\varDelta p(t) \!\mid\!  \varDelta p\left(t_{0}: t-\delta t \right), \varDelta \omega\right) =                                           \\
		 & \int \bigl(\mathcal{P}(\varDelta p(t) \!\mid\!  \varDelta f(t), \varDelta \omega)  \mathcal{P}\!\left(\varDelta \!f(t) \!\mid\!  \varDelta p\!\left(t_{0}\!: \! t-\delta t\right), \varDelta \omega\right) \bigr) d \varDelta f(t) \notag
	\end{align}
\end{newsubequations}
