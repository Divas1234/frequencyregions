...
Since the posterior of frequency derivation considering uncertain variability factors can be accurately solved through the Bayesian filter method, a chance-constrained nadir constraint can be derived as follows,
...
\noindent
where $\varepsilon_{SFR}$ is the confidence coefficient associated with \eqref{eqn:13}; \(\varDelta f(t)\) can be generated from its posterior distribution denoted by \eqref{eqn:13}. Using sampled average approximate (SAA) method, \cref{eqn:13} can be reshaped as the deterministic form \eqref{eqn:14a}, which is listed below,

\vspace{-0.125in}
\begin{small}
	\begin{align}
		\left(\varDelta f(t)\right)^{(n)} - M \tau_{n}\!           & \leq \!\varDelta f_{nadir}, \notag\\
        &\!\!\!\!\!\!\!\!\!\!\!\!\!\!\! \forall n \in\{1,2,...n,...,N\},\forall t\in(t_{IR}, t_{PFR}] \label{eqn:14a} \\
		\frac{1}{{N}}\sum\nolimits_{n\in\{1,2,...n,...,N\}}  \tau_{n} & \leq \varepsilon_{SFR},\forall n \in \{1,2,...n,...,N\} \label{eqn:14b}
	\end{align}
\end{small}

\noindent
where \(\tau_{SFR}\) is a binary indicator describing whether \((\varDelta f(t))^{(n)}\) satisfying the frequency derivation requirement, e.g., if \((\varDelta f (t))^{(n)}\leq \varDelta f_{nadir}\), then  \(\tau_{SFR}=0\) and otherwise  \(\tau_{SFR}=1\).
