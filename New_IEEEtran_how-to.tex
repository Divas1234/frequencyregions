...
Homogeneous generators are usually treated as equivalent units with aggregated parameters for simplification. Therefore, the union frequency dynamic for the system consisting of conventional generators, converters-interfaced generators (renewable energy), and battery energy systems (BESS) can be rewritten as,
...
\noindent where \(\textit{likelihood function}\) in \cref{eqn:5} declares coupling properties between \(\varDelta p(t)\) and \(\varDelta f(t)\); \(\textit{prior function}\) describes the recursive process from \(\varDelta f(t_0: t-\delta t)\) to \(\varDelta f(t)\). Using the prior and predicated steps of the Bayesian filter written probabilistically, the influence of \(\varDelta \omega\) can be well incorporated over the entire frequency dynamic process.
In which, \(\textit{prior function}\) can be recursively calculated as follows,

% \vspace{-0.125in}
\begin{newsubequations}\label{eqn:6}
	\begin{align}
		 & \mathcal{P}\left(\varDelta f(t) \!\mid\!  \varDelta p\left(t_{0}: t-\delta t \right), \varDelta \omega\right) =                                           \\
		 & \int \bigl(\mathcal{P}(\varDelta f(t) \!\mid\!  \varDelta f(t_0:t-\delta t), \varDelta \omega)  \mathcal{P}\!\left(\varDelta \!f(t-\delta t) \!\mid\!  \varDelta p\!\left(t_{0}\!: \! t-\delta t\right), \varDelta \omega\right) \bigr) d \varDelta f(t) \notag
	\end{align}
\end{newsubequations}
