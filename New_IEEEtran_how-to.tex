...
In \cref{eqn:5a,eqn:6a}, \textit{prior function} and \textit{posterior function} can not be directly solved because their distributions are unknown. Therefore, the recursive SIS algorithm is used to address this issue.
...
By combining \eqref{eqn:9a} and \eqref{eqn:7a}, the posterior of the importance density in \eqref{eqn:7c} can be directly derived. Subsequently, by updating the weight through \eqref{eqn:7b} and sequentially substituting \cref{eqn:7b,eqn:7c} into \eqref{eqn:7a}, the posterior distribution of \(\varDelta f(t)\) in \eqref{eqn:5a} can be recursively solved. 
%For intuitively illustrating the implementing process of the posterior distribution of \(\varDelta f(t)\).
...
This section examines frequency requirements, i.e., RoCoF, nadir, and frequency containment reserve (FCR), among them, a refined chance-constrained formulation of frequency nadir constraint is derived based on the BF-SFR described in section II; remaining constraints concerning RoCoF/FCR can be found in \cite{restrepo2005unit, teng2015stochastic, chu2020towards, ding2021two,zhang2020modeling, zhang2021encoding,oskouee2021primary,perroy2019provision,wang2021adaptive}.
...
The RoCoF constraint denoted by \(\varDelta f^{\prime} (t)\) aims to mitigate the possible rapid changes in the inertia response (IR).
% It is required that $\varDelta f^{\prime}(t)$ does not exceed the maximum rate of change of frequency, denoted as $RoCoF_{\text{max}}$, \textit{i.e.,} $\varDelta f^{\prime}(t)\!\leq \! RoCoF_{\text{max}}$. 
Since the maximum of \(\varDelta f^{\prime}(t)\) equals to $\nicefrac{\varDelta p_{\text{step} }}{M_{\varSigma}}$, \cite{garcia2021requirements}, and then, the RoCoF constraint can be expressed,
...
\begin{small}
	\begin{align}\label{eqn:10}
		2{H_{\varSigma}^{-1}}{\varDelta p_{step}}      & \leq \overline{\gamma}_{rocof}, \forall g, t \rightarrow                                                                                                   \\
...
Next, the FCR constraint aims to ensure sufficient frequency reserves provided by homogeneous generators concerning additional output at primary frequency response (PFR). By taking \cref{eqn:3a,eqn:3b,eqn:2f} into consideration, the FCR constraint is expressed as follows:
...
\begin{newsubequations}\label{eqn:11}
	\begin{align}
		\sum\nolimits_{w\in\mathcal{W}} \left( \overline{p}_{w} - p_{w, t} \right)                     & \geq \varDelta p_{W}(t), \forall t\in(t_{IR},t_{PFR}] \label{eqn:11a}                                     \\
		...
		\sum\nolimits_{g\in\mathcal{G}} \left( \overline{p}_{g} -p_{g, t}\right)                       & \geq \varDelta p_{step},\forall t\in(t_{IR}, t_{PFR}] \label{eqn:11d}
	\end{align}
\end{newsubequations}

\noindent
where the \(t_{IR},t_{PFR}\), respectively, denotes the initial and final time of the PFR. Let \(\langle \overline{\varDelta p_{W}}, \overline{\varDelta p_{E}}, \overline{\varDelta p_{G}}\rangle\) denotes, respectively, the maximum of \(\varDelta p_{W}(t), \varDelta p_{E}(t), \varDelta p_{G}(t)\), a conservative form of \cref{eqn:11a,eqn:11b,eqn:11c} can be calculated as follows:

\vspace{-0.125in}
\begin{newsubequations}\label{eqn:11}
	\begin{align}
		\sum\nolimits_{w\in\mathcal{W}} \left( \overline{p}_{w} - p_{w, t} \right)                     & \geq \overline{\varDelta p_{W}}, \notag                                                                                                                                                                       \\
		                                                                                               & \!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\! \overline{\varDelta p_{W}} = (K_W R_W^{-1} + D_{W})\varDelta f_{nadir}, \forall t\in(t_{IR},t_{PFR}] \label{eqn:12a} \\
		...
		\sum\nolimits_{g\in\mathcal{G}} \left( \overline{p}_{g} -p_{g, t}\right)                       & \geq \overline{\varDelta p_{G}}, \notag                                                                                                                                                                       \\
		                                                                                               & \!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\! \overline{\varDelta p_{G}} = K_G R_G^{-1} \varDelta f_{nadir},\forall t\in(t_{IR}, t_{PFR}]\label{eqn:12c}
	\end{align}
\end{newsubequations}

% where \(\overline{\varDelta p_{W}} = (K_W R_W^{-1} + D_{W})\varDelta f_{nadir}\), \(\overline{\varDelta p_{E}} = R_E^{-1}\varDelta f_{nadir}\), and \(\overline{\varDelta p_{G}} = K_G R_G^{-1} \varDelta f_{nadir}\).
...
%Again, in the context of the $FCR$ constraint \eqref{eqn:11c} of conventional generators, the right-hand side of \eqref{eqn:11c}, representing the adjustable output of conventional generators, can be rewritten as
...
The frequency nadir constraint aims to avoid \(\varDelta f(t)\)  beyond its predefined threshold \(\varDelta f_{nadir}\) at the PFR process.
Since the posterior of frequency derivation considering uncertain variability factors can be accurately solved through the Bayesian filter method, a chance-constrained frequency nadir constraint can be derived as follows,

\vspace{-0.125in}
\begin{small}
	\begin{align}\label{eqn:13}
		 & \textrm{Prob}\left(\varDelta f(t) \leq \varDelta f_{nadir}\right) \geq 1-\varepsilon_{SFR};\forall t\in(t_{IR}, t_{PFR}] \notag \\
		 & \qquad\,\,\,\, \varDelta f(t) \propto \mathcal{P}(\varDelta f(t)|\varDelta p(t_0:t); \varDelta \omega)
	\end{align}
\end{small}
...
\noindent
where $\varepsilon_{SFR}$ is the confidence coefficient associated with \eqref{eqn:13}; \(\varDelta f(t)\) can be generated from its posterior distribution denoted by \eqref{eqn:13}. Using sampled average approximate (SAA) method, \cref{eqn:13} can be equivalently reshaped as the deterministic form \eqref{eqn:14a}, which is listed below,

\vspace{-0.125in}
\begin{newsubequations}
	\begin{small}
		\begin{align}
			\left(\varDelta f(t)\right)^{(n)} - \varXi \tau_{n}\!              & \leq \!\varDelta f_{nadir}, \label{eqn:14a}                             \\
			\forall n                                                     & \in\{1,2,...n,...,N\},\forall t\in(t_{IR}, t_{PFR}]  \notag             \\
			\frac{1}{{N}}\sum\nolimits_{n\in\{1,2,...n,...,N\}}  \tau_{n} & \leq \varepsilon_{SFR},\forall n \in \{1,2,...n,...,N\} \label{eqn:14b}
		\end{align}
	\end{small}
\end{newsubequations}
...
\vspace{-0.125in}
\noindent
where \((\varDelta f(t))^{(n)}\), following its posterior \eqref{eqn:7a}, enforces possible realization of \(\varDelta f(t)\). \(\tau_{SFR}\) denotes a binary indicator describing whether \((\varDelta f(t))^{(n)}\) satisfying the frequency security requirement, e.g., if \((\varDelta f (t))^{(n)}\!\leq\! \varDelta f_{nadir}\), then  \(\tau_{SFR}\!=\!0\), and vice versa. Despite that the posterior function \eqref{eqn:7a} depends on the initial mismatch power \(\varDelta p_{step}\), equivalent frequency parameters \(\langle H_{\varSigma}, D_{\varSigma}\rangle\), and aggregated generators' parameters \(\{\langle R_{G},F_{G},K_{G}\rangle), \langle K_{W},R_{W}\rangle, R_{E}\}\), and various uncertain variability factors \(\varDelta \omega(t)\). Therefore, \cref{eqn:7a} can be mathematically described as the nonlinear relation of the variables above. Furthermore, to prevent introducing nonlinear terms purposes, a linearized fitting function soft-linking to frequency parameters is reconstructed as \eqref{eqn:7a} to approximate the posterior function.

\vspace{-0.125in}
\begin{newsubequations}
	\begin{small}
		\begin{align}
			 & \varDelta  f(t) =\!\!\!\!\!\! \max\limits_{m\in\{1,2,...,m,...,M\}}\left(\alpha_{m}^{(1)} H_{\varSigma} +  \alpha_{m}^{(2)} D_{\varSigma} + \alpha_{m}^{(3)} \left(\frac{F_{G}}{R_{G}}\right)+\right.\notag \\
			 & \left. \alpha_{m}^{(4)}\right):H_{\varSigma} \!=\! H_{G} + H_{W}, D_{\varSigma} \!=\! D_{G}+D_{W}+{ R_{W}^{-1} } + { R_{E}^{-1}} \label{eqn:15a}
		\end{align}
	\end{small}
\end{newsubequations}

%$\varDelta f_{nadir}$ is closely dependent with the initial imbalance disturbance $\varDelta p_{\text{step}}$, aggregated frequency parameters of various grid-connected units $\langle M_{G},\! D_{G}, \!R_{G}, \!K_{G}, \!T_{G}, R_{W},$ $R_{E}\rangle $ and external UV characteristic $\langle \mu_{1}, \sigma_{1}, \mu_{2}, \sigma_{2}\rangle $. For simplification purpose, let ${\wp}=\langle M_{G},$ $\! D_{G}, \!R_{G}, \!K_{G}, \!T_{G}, R_{W},\!R_{E}\rangle $ and ${\zeta} = \langle \mu_{1}, \sigma_{1}, \mu_{2}, \sigma_{2}\rangle $, and then, $\varDelta f_{\text{nadir}}$ can be  rewritten a nonlinear mapping function $\Psi(\wp, \zeta)$, $i.e.$, $\varDelta f_{\text{nadir}} = \Psi(\wp, \zeta)$. Furthermore, a Sampled Average Approximate (SAA) method is used to construct the deterministic convex formulation of \eqref{eqn:15}, which can be readily identified by any off-the-shelf MIP solvers and subsequently integrated into UC. To achieve that, by introducing a binary indicator variable $z_{q}\in\{0,1\} \!:\!q\!\in\!\{1, 2,\ldots, N\!Q\}$ and using the Big-M method \cite{ding2021two,zhang2020modeling},  \cref{eqn:15} can be equivalently reformulated with a set of linearized constraints as follows,
%
...
% Let $\hat{\Psi}(\wp, \hat{\zeta}^{(q)}) \!=\!\max\left(\alpha_{\iota,1}^{(q)}\tilde{H}_{\text{equ}}^{(q)} + \alpha_{\iota,2}^{(q)} \tilde{D}_{\text{equ}}^{(q)} + \alpha_{\iota,3}^{(q)}\left(\frac{\tilde{F}_G^{(q)}}{\tilde{R}_G^{(q)} }\right)\right.$ $\left. + \alpha_{\iota,4}^{(q)} \right) $ describe the approximated fitting function of $\hat{\Psi}(\wp, \hat{\zeta}^{(q)})$ given a stochastic uncertain realization $\hat{\zeta}^{(q)}$; $\langle \tilde{H}_{\text{equ}}^{(q)}, \tilde{D}_{\text{equ}}^{(q)}, \tilde{F}_G^{(q)}$, $\tilde{R}_G^{(q)}$ denotes a possible set of aggregated frequency parameters with the respect of $\hat{\zeta}^{(q)}$. In such a situation, $\hat{\Psi}(\wp, \hat{\zeta}^{(q)})$ can be evaluated by observing the maximum value of constructed hyperplane $\Theta_{\iota\in\{1,2,3,4\}}^{(q)}$ where $\Theta_{\iota}^{(q)} = \alpha_{\iota,1}^{(q)}\tilde{H}_{\text{equ}}^{(q)} + \alpha_{\iota,2}^{(q)} \tilde{D}_{\text{equ}}^{(q)} + \alpha_{\iota,3}^{(q)}\left(\frac{\tilde{F}_G^{(q)}}{\tilde{R}_G^{(q)} }\right)$. Particularly, the selection of fitting parameters $\langle \alpha_{\iota, 1}^{(q)},\alpha_{\iota, 2}^{(q)}, \alpha_{\iota, 3}^{(q)}, \alpha_{\iota, 4}^{(q)}  \rangle$ can be solved by minimizing the loose function defined by $\vartheta_{\text{loss}} \!=\! \hat{\Psi}(\wp, \hat{\zeta}^{(q)}) \!-\! \Psi(\wp, \hat{\zeta}^{(q)})$. As such, the below lists a detailed optimization model for calculating fitting parameters.
\noindent
where \(\alpha_{m}^{(i)}:i\in\{1,2,3,4\}\) are fitting parameters of the \(m\)-th hyperplane. Using \cref{eqn:15a}, the maximum frequency derivation can be estimated by automatically selecting the nearest hyperplane to the posterior \eqref{eqn:7a}. Following the discussion mentioned, the associated optimization model concerning fitting parameters can be constructed as below,

\vspace{-0.125in}
\begin{small}
	\begin{align}\label{eqn:16}
		\min & \sum\nolimits_{r\in\mathcal{R}} \Biggl\{\max\limits_{m\in\{1,2,...,m,...,M\}} \Bigl(\alpha_{m}^{(1)}{H}_{\varSigma}^{(n)} + \alpha_{m}^{(2)} {D}_{\varSigma}^{(n)} + \\
...
In \cref{eqn:16}, the inner $``\max"$ operator automatically selects the most suitable hyperplane closet to each evaluation point, and the outer $``\min"$ operator guarantees the global accuracy between the pre-selected hyperplane and all evaluation points. In terms of problem-solving, \cref{eqn:16} can be solved by directly calling \code{Ipopt} solver or reformulating it as its recursive form and subsequently invoking existing MIP solver, e.g., \code{Gurobi}, \code{IBM CPLEX}, etc. More detailed on the calculation of \eqref{eqn:16} can be found in \cite{ding2021two,zhang2020modeling}
...
Next, substituting the equivalent form in \eqref{eqn:15a} concerning \(\varDelta f(t)\) derived from \eqref{eqn:16} into \cref{eqn:14a,eqn:14b}, a convex form of frequency nadir constraint can be obtained as follows,

\vspace{-0.125in}
\begin{newsubequations}
	\begin{small}
		\begin{align}
			& \max\limits_{m\in\{1,2,...,m,...,M\}}\left(\alpha_{m}^{(1)} H_{\varSigma} +  \alpha_{m}^{(2)} D_{\varSigma} + \alpha_{m}^{(3)} \left(\frac{F_{G}}{R_{G}}\right) + \alpha_{m}^{(4)}\right)- \notag \\
			& \varXi \tau_{n}\!  \leq \varDelta f_{nadir}, \forall n \in\{1,2,...n,...,N\},\forall t\in(t_{IR}, t_{PFR}]  \label{eqn:17a} \\
			& \frac{1}{{N}}\sum\nolimits_{n\in\{1,2,...n,...,N\}}  \tau_{n} \leq \varepsilon_{SFR},\forall n \in \{1,2,...n,...,N\} \label{eqn:17b}
		\end{align}
	\end{small}
\end{newsubequations}
...
\vspace{-0.125in}
\section{The proposed FCUC model}

This section presents a comprehensive articulation of the proposed FCUC in this work. The form of the suggested UC model can be constructed by adding frequency constraints from section III into the baseline UC studies.

\vspace{-0.125in}
\begin{newsubequations}\label{eqn:18}
	\begin{align}
		 & Objective\,\, function: \min\sum\nolimits_{t\in\mathcal{T}} \!\!\Bigg\{ \! \sum\nolimits_{g\in\mathcal{G}}\Bigl( C_{g}^{{up}} u_{g,t} \!+\! C_{g}^{{dw}} v_{g,t} +\notag                                  \\
		 &C_{g}^{nl} x_{g,t}\Bigr) + \sum\nolimits_{s\in\mathcal{S}} \mathbf{Prob}_{s} \biggl(\sum\nolimits_{g\in\mathcal{G}} \sum\nolimits_{z\in\mathcal{Z}} \left(\varsigma_{g,z}^{(1)} p_{g,s,t}^{(z)} + \varsigma_{g,z}^{(2)}\right) + \notag \\
        &\qquad\qquad\qquad\quad\,\,\, \sum\nolimits_{d \in\mathcal{D}} C_{{VoLL}}\varDelta p_{d,s,t}\biggr)\Biggr\}\label{18-a}\\
         ...
         & \sum\nolimits_{g\in\mathcal{G}} H_{g} x_{g,t} \!\geq\! \frac{1}{2}{\varDelta p_{{step} }}{\overline{\gamma}_{rocof}^{-1}} \!-\! \sum\nolimits_{w\in\mathcal{W}} H_{w},\forall  t \qquad \,\, \label{18-u}                        \\
...
\begin{newsubequations}\label{eqn:21}
	\begin{align}
		 &\max\limits_{m\in\{1,2,...,m,...,M\}}\left(\alpha_{m}^{(1)} H_{\varSigma,t} +  \alpha_{m}^{(2)} D_{\varSigma,t} + \alpha_{m}^{(3)} \left(\frac{F_{G,t}}{R_{G,t}}\right) +\right. \notag \\
         ...
         &R_{E,t}\in\mathbb{R}^{+},F_{G,t}\in\mathbb{R}^{+}  \label{19-k}
	\end{align}
\end{newsubequations}
...
In the above formulation, \cref{18-a} denotes the objective function of the FCUC problem; it seeks to minimize the overall cost associated with conventional generators. 
Constraints \labelcref{18-b,18-c} specify conventional generators' minimum up-time and down-time requirements, respectively. 
Constraints \labelcref{18-d,18-e} ensure the coherence of the shut-up/shut-down choice and the operational condition of any individual conventional generator. 
Constraint \labelcref{18-f} delineates the constraint about power balance restriction. 
Constraint \labelcref{18-g} denotes the constraint that represents the capacity of each transmission line. 
Constraints \labelcref{18-h,18-i} ensure the compliance of each conventional generator's up-forward and down-forward ramping rates throughout consecutive dispatching periods.
Constraints \labelcref{18-j,18-k} delineate the stipulation for providing up-forward and down-forward spinning reserve capacity generated by conventional generators. 
Constraint \eqref{18-l} indicates the minimize/minimize output of conventional generators. 
Constraints \labelcref{18-m,18-n} introduce limitations on actual wind output and forced-load-curtailment. 
Constraints \labelcref{18-o,18-p} specify the binding of charging/discharging capacities for each BESS. 
Constraint \labelcref{18-q} signifies a coupling restriction between the energy of BESS and the corresponding discharging/charging decisions.
Constraint \labelcref{18-r} establishes the maximum cumulative charging/discharge amount for each BESS.
Constraint \labelcref{18-s} underscores the exclusiveness of the discharging and charging choices made by each BESS.
Constraint \labelcref{18-t} enforces that the output of the conventional generator is equal to the total of the segmented outputs generated on each block of its linearized fuel cost curve. 
Constraints \labelcref{18-u} defines the security requirement of the RoCoF under imbalanced disturbances.
Constraints \labelcref{18-v,18-w,18-x,18-y} denote the additional output of multiple generators participating in the frequency-supporting process.
Constraints \labelcref{18-z} represent the binding of FCR.
Constraints \labelcref{19-a,19-b,19-c,19-d,19-e,19-f,19-g,19-h,19-i} list the aggregated frequency parameters of homogeneous generators concerning the current operation status.
Constraints \labelcref{19-j,19-k} describe the feasible region of binary and continuous decision variables, respectively.
