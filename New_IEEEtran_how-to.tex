...
1) a new SFR model considering uncertain variability factors from converter-interfaced generators is proposed. The proposed model can estimate frequency derivation and associated probabilistic distribution against mismatch power disturbance.
...
For the sake of simplification, homogeneous generators with similar controls usually treated as equivalent units with aggregated parameters. Therefore, the union frequency dynamic for the system consisting of conventional generators, converters-interfaced generators and battery energy systems (BESS), can be written as,
...
In \eqref{time-discretized SFR: additional power from convertional units}, the uncertain variability factors are conditionally depended on frequency derivation and itself actual output restriction, to reflect this, let \(\varDelta \omega(t) = \lceil H(\varDelta f(t)), \overline{p}_{add,W}\rceil + \sigma_{add,W}\) define the uncertain variability factors. the form of SFR considering uncertain variability factors can be obtained as below,
...
\begin{newsubequations}
	\begin{align}
		 & \mathcal{P}(\varDelta f(t+\delta t) | \varDelta p(t_0: t+\delta t; \mu_1,\sigma_1))=                                                                                                                                                          \\
...
\noindent where \cref{time-discretized SFR: swing equation} declares the change of \(\varDelta f(t)\) between adjacent discretized steps; \cref{time-discretized SFR: mismatch power} describes chronological mismatch power; \cref{time-discretized SFR: additional power from conventional units,time-discretized SFR: additional power from converters,additional power from bess}  enforces rescheduled outputs produced by aggregated conventional units, aggregated wind units and aggregated battery energy units.
