...
This section examines frequency requirements, i.e., RoCoF, nadir, and frequency containment reserve (FCR), and these needs are reevaluated into constraints and incorporated into UC studies. Among them, a new chance-constrained formulation of frequency nadir constraint is derived based on the BF-SFR; remaining constraints concerning RoCoF/FCR can be found in \cite{restrepo2005unit, teng2015stochastic, chu2020towards, ding2021two,zhang2020modeling, zhang2021encoding,oskouee2021primary,perroy2019provision,wang2021adaptive}.
...
The RoCoF denoted by \(\varDelta f^{\prime} (t)\) aims to mitigate the possible rapid changes in the inertia response (IR).
...
Since the maximum of \(\varDelta f^{\prime}(t)\) equals to $\nicefrac{\varDelta p_{\text{step} }}{M_{\varSigma}}$, \cite{garcia2021requirements}, so, the RoCoF constraint can be expressed,
...
\begin{newsubequations}\label{eqn:11}
	\begin{align}
		 \sum\nolimits_{w\in\mathcal{W}} \left( \overline{p}_{w} - p_{w, t} \right) &\geq \varDelta p_{W}(t), t\in(t_{IR},t_{PFR}] \label{eqn:11a}                                                                  \\
		 \sum\nolimits_{e\in\mathcal{E}} \left( \overline{p}_{e} u_{e,t}^{dis} -p_{e, t}^{{dis}}\right)&\geq \varDelta p_{E}(t), t\in(t_{IR}, t_{PFR}] \label{eqn:11b}                                                     \\
		 \sum\nolimits_{g\in\mathcal{G}} \left( \overline{p}_{g} -p_{g, t}\right)&\geq \varDelta p_{G}(t), t\in(t_{IR}, t_{PFR}]\label{eqn:11c}                                              
	\end{align}
\end{newsubequations}

\noindent
where the \(t_{IR},t_{PFR}\), respectively, denotes the initial and final time of the PFR. Let \(\langle \overline{\varDelta p_{W}}, \overline{\varDelta p_{E}}, \overline{\varDelta p_{G}}\rangle\) denotes, respectively, the maximum of \(\varDelta p_{W}(t), \varDelta p_{E}(t), \varDelta p_{G}(t)\), a concise form of \cref{eqn:11a,eqn:11b,eqn:11c} can be calculated as follows:
