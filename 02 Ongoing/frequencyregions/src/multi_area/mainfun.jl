...
function save_plot_if_present(plot_obj, path::String)
	if !isnothing(plot_obj)
		...
		if endswith(path, ".pdf")
			png_path = replace(path, ".pdf" => ".png")
			Plots.savefig(plot_obj, png_path)
			println("Saved: $path and $png_path")
		else
			println("Saved: $path")
		end
	end
...
function run_multiarea_analysis(;
	...
	# Time-domain mutual support trajectories verification plot
	p_trajectories = plot_multiarea_mutual_support_trajectories(sys, comp_cfg)
	...
	save_plot_if_present(p_trajectories, "fig/multi_area/multiarea_mutual_support_trajectories.pdf")
	...
	println("Mutual support trajectories plot: fig/multi_area/multiarea_mutual_support_trajectories.pdf")
	println("=== Done ===")
	...
	return (
		...
		mutual_support_trajectories_plot = p_trajectories,
