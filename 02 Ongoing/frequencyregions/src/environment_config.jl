...
# ===== Multi-Area Analysis Module =====
include("multi_area/network_topology.jl")
include("multi_area/decoupled_workflow.jl")
include("multi_area/multi_area_viz.jl")
include("multi_area/multi_area_runner.jl")
...
function calculate_vertex(DAMPING_RANGE, inertia_updown_bindings, fittingparameters,
	...
	fit_curve = fittingparameters[1] .+ fittingparameters[2] .* damp_vals .+ fittingparameters[3] .* damp_vals .^ 2

	vertexs = Tuple{Float64, Float64, Float64}[]

	# Bottom boundary: from min_damping to max_damping
	for i in 1:n_points
		d = damp_vals[i]
		h_bot = max(lower_bounds[i], min_inertia, fit_curve[i])
		push!(vertexs, (Float64(droop), Float64(d), Float64(h_bot)))
	end

	# Top boundary: from max_damping back to min_damping
	for i in n_points:-1:1
		d = damp_vals[i]
		h_top = upper_bounds[i]
		push!(vertexs, (Float64(droop), Float64(d), Float64(h_top)))
	end
