using Plots
using LinearAlgebra
# Potentially needed for fitting or other calculations, good practice to include if used by dependencies
...
function generate_inertia_damping_figure(
    ...
    controller_configs = converter_forming_configurations()
    ...
    initial_inertia, factor_coeff, time_const, _, rocof_limit, nadir_limit, power_dev = get_parameters(flag_converter)
...
# --- Example Usage ---
# Ensure the necessary helper functions (converter_forming_configurations, etc.) and
