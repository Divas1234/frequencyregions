@testset "multi-area topology" begin
    ...
    @test compute_tie_line_contribution(1, system; factor = 0.5) == 0.05
    @test compute_tie_line_contribution(2, system; factor = 1.0) == 0.1

    # Test asymmetric resources system
    sys_asym = build_asymmetric_resources_system()
    @test length(sys_asym.areas) == 2
    @test sys_asym.areas[1].droop == 18.0
    @test sys_asym.areas[2].droop == 40.0
    @test sys_asym.areas[1].time_constant == 0.5
    @test sys_asym.areas[2].time_constant == 0.2
    ...
    @test sys_strong.areas[1].droop == 40.0
    @test sys_strong.areas[2].droop == 18.0
    @test sys_strong.areas[1].time_constant == 0.2
