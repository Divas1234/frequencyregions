...
gr()
ENV["GKSwstype"] = "100" # Force headless GR for robust file saving
# theme(:wong2)

# ===== Global Constants =====
# Define constants BEFORE including modules that use them
...
# ===== Core Configuration and Structures =====
include("sys_boundary_parameters.jl")
include("converter_config.jl")
include("config_structures.jl")
include("validation.jl")

# ===== Computational Modules =====
include("inertia_rocof_estimation.jl")
include("core_inertia_damping_calculation.jl")
include("frequency_response_analysis.jl")
include("inertia_damping_fitting.jl")

# ===== Visualization Modules =====
include("visualization_generator.jl")
include("polyhedra_geometry_3d.jl")
include("polygon_visualization_2d.jl")
...
include("multi_area_analysis.jl")
include("visualization_multi_area.jl")
...
function sub_data_visualization(
	...
	try
		return plot_inertia_distribution_with_bounds_improved(
			interaction_point,
			damping,
			bounds_mat,
			max_inertia,
...
function calculate_vertex(DAMPING_RANGE, inertia_updown_bindings, fittingparameters,
	...
	if !isnothing(interaction_index_1) && !isnothing(interaction_index_2)
		inertia_1 = fittingparameters[1] .+ fittingparameters[2] .* DAMPING_RANGE[interaction_index_1] .+ fittingparameters[3] .* DAMPING_RANGE[interaction_index_1] .^ 2
		inertia_2 = fittingparameters[1] .+ fittingparameters[2] .* DAMPING_RANGE[interaction_index_2] .+ fittingparameters[3] .* DAMPING_RANGE[interaction_index_2] .^ 2

		push!(
			vertexs,
			create_vertex(
				droop, DAMPING_RANGE[interaction_index_1], inertia_1,
			),
		)
		push!(
			vertexs,
			create_vertex(
				droop, DAMPING_RANGE[interaction_index_2], inertia_2,
			),
		)
	end
