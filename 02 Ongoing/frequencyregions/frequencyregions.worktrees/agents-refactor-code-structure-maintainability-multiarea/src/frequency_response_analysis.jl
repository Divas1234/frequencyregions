...
function generate_extreme_inertia(
	...
	for i in eachindex(damping)
		...
		if id !== nothing
			...
			# Fall back to the highest candidate inertia to keep the workflow robust
			# for challenging parameter combinations (e.g., multi-area aggregation).
			extreme_inertia[i] = candidate_inertia[end]
			selected_ids[i] = length(candidate_inertia)
			@warn "No nadir crossing found at damping=$(damping[i]); using boundary inertia fallback."
		end
	end
