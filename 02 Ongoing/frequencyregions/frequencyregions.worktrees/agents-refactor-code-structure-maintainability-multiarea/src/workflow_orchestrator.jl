...
```
"""
function execute_workflow(system_params::SystemParameters, config::ComputationConfig, 
...
function execute_workflow(droop::Float64, config::ComputationConfig, 
    ...
    base_params = create_system_parameters(config.flag_converter)
    
    # Create system params with overridden droop
    system_params = SystemParameters(
        base_params.initial_inertia,
        base_params.factorial_coefficient,
        base_params.time_constant,
        droop,
        base_params.rocof_threshold,
        base_params.nadir_threshold,
        base_params.power_deviation
    )
    
    return execute_workflow(system_params, config, controller_config)
end
