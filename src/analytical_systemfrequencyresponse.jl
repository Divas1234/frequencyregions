...
function generate_extreme_inertia(
	...
	nadir_vector, inertia_vector, extreme_inertia, selected_ids = zeros(length(damping), ll),
	zeros(length(damping), ll), zeros(length(damping), 1), zeros(length(damping), 1)

	frequency_nadir_threshold = 0.25
...
function calculate_frequencynadir(
	...
	res = delta_p / (damping + droop) *
		  (1 +
		   (-1)^(k) * sqrt(time_content * (droop - factorial_coefficient) / (2 * inertia)) *
		   exp(-1 * ζ * ωₙ * t))
